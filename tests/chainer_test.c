/** @file chainer_test.c
 * @brief Chainer Unit Tests
 *
 * Comprehensive test suite for the Chainer pipeline system.
 * Tests cover creation, filter/map/skip/take stages, terminals,
...
int test_chainer_multiple_filters(void) {
    Vector *vec = create_mixed_vector(20);
    Chainer c = Chain((Container *)vec);
    chain_filter(&c, is_positive);
    chain_filter(&c, is_even);
    size_t count = chain_count(&c);
    size_t expected = 0;
    for (int i = -9; i <= 10; i++) {
        if (i > 0 && i % 2 == 0) expected++;
    }
    ASSERT_EQUAL(count, expected, "multiple filters wrong count");
    chain_destroy(&c);
    vector_destroy(vec);
    return 1;
}
...
/* ============================================================================
 * Terminal: chain_collect_into (HashMap)
 * ============================================================================ */

int test_chainer_collect_intoto_hashmap(void) {
    ...
    /* Build pipeline: map strings to pairs */
    Chainer c = Chain((Container *)src);
    chain_map(&c, make_pair, sizeof(StrPair));

    /* Collect into HashMap */
    HashMap *map = hashmap_str_str();
    size_t n = chain_collect_into(&c, (Container *)map);
...
int test_chainer_collect_intoto_hashmap_empty(void) {
    Vector *src = vector_str();
    Chainer c = Chain((Container *)src);
    chain_map(&c, make_pair, sizeof(StrPair));
...
int test_chainer_collect_intoto_hashmap_with_duplicates(void) {
    Vector *src = vector_str();
...
/* ============================================================================
 * Terminal: chain_collect_into (Vector)
 * ============================================================================ */
...
int main(void) {
    ...
    TEST(test_chainer_collect_intoto_hashmap);
    TEST(test_chainer_collect_intoto_hashmap_empty);
    TEST(test_chainer_collect_intoto_hashmap_with_duplicates);
